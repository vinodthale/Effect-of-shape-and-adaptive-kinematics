#!/bin/bash
# Generate all 48 Zhang et al. (2018) input files
# 8 Reynolds numbers × 6 thickness ratios = 48 cases

# Reynolds number values from Zhang (2018)
RE_VALUES=(50 500 1000 5000 10000 50000 100000 200000)

# Thickness ratios (NACA 00XX)
H_VALUES=(004 008 012 016 020 024)
H_DECIMAL=(0.04 0.08 0.12 0.16 0.20 0.24)

# Grid and timestep settings by Re regime
get_settings() {
    local re=$1

    if [ $re -lt 1000 ]; then
        # Low Re: finer grid, smaller timestep
        echo "128 0.00005 0.2 20.0"
    elif [ $re -lt 10000 ]; then
        # Moderate Re
        echo "64 0.0001 0.3 10.0"
    else
        # High Re: may need finer resolution
        echo "64 0.00005 0.25 8.0"
    fi
}
...
echo "Generating Zhang et al. (2018) input files..."
echo "Total: 48 files (8 Re × 6 thickness)"
echo ""
...
for i in "${!RE_VALUES[@]}"; do
    ...
    for j in "${!H_VALUES[@]}"; do
        h=${H_VALUES[$j]}
        h_dec=${H_DECIMAL[$j]}

        filename="input2d_Zhang_Re${re}_h${h}"
        ...
        # Get Re-specific settings
        read N DT_MAX CFL_MAX END_TIME <<< $(get_settings $re)

        count=$((count + 1))
        echo "Creating ($count/48): $filename (Re=$re, h/L=$h_dec)"

        # Generate input file
        cat > "$filename" <<EOF
// Zhang et al. (2018) - Strict Reproduction
// Reynolds Number: Re = $re
// Thickness ratio: h/L = $h_dec (NACA00${h})
// Reference: Physics of Fluids 30, 071902 (2018)

// Physical parameters
Re = ${re}.0
MU = 0.785/Re        // Viscosity: ν = V_max / Re
RHO = 1.0
...
ConstraintIBMethod {
   ...
   output_dirname        = "./Zhang_Results_Re${re}_h${h}"
   base_filename         = "Zhang_Re${re}_h${h}"
  }
}
...
eel2d {
     ...
     // ===== ZHANG (2018) FIXED PARAMETERS =====
     // DO NOT MODIFY - Required for Zhang reproduction
     reynolds_number                   = ${re}.0
     thickness_ratio                   = $h_dec
     base_amplitude                    = 0.125    // A_max (FIXED)
     base_frequency                    = 0.785    // V_max = 0.785
     swimming_mode                     = 0.0      // Anguilliform

     // CRITICAL: Disable adaptation for Zhang mode
     enable_shape_adaptation           = FALSE
     envelope_power                    = 1.0      // Linear envelope (FIXED)
     tail_width_ratio                  = 0.02

     // Performance tracking
     track_performance                 = TRUE
     performance_log_file              = "Zhang_performance_Re${re}_h${h}.dat"

     // Zhang (2018) prescribed kinematics
...
Main {
   ...
   log_file_name = "Zhang_Re${re}_h${h}.log"
   log_all_nodes = FALSE
   viz_writer = "VisIt"
   viz_dump_interval = 50
   viz_dump_dirname = "Zhang_viz_Re${re}_h${h}"
...
echo "Created $count new input files"
echo "Total files in directory: $(ls -1 input2d_Zhang_* 2>/dev/null | wc -l)"
echo ""
echo "To run all cases:"
echo "  for f in input2d_Zhang_*; do mpirun -np 6 ../../build/main2d \$f; done"
